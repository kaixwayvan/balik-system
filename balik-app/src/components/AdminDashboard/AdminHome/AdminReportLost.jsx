...
export default function AdminReportLost({ isOpen, onClose }) {
  const { user } = useAuth();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [formData, setFormData] = useState(initialLostFormData);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const mappedData = {
        ...
        date_lost: new Date(formData.dateLost).toISOString(),
        status: 'pending',
        image_url: imageUrl,
        metadata: {
      ...
      alert("Lost item report encoded successfully!");
      onClose();
      setFormData(initialLostFormData);
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-2 gap-6 mb-4">
            ...
            <div>
              ...
              <input type="text" name="whatWasLost" value={formData.whatWasLost} onChange={handleChange} required className="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:border-blue-500" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-6 mb-4">
            ...
            <div>
              ...
              <input type="date" name="dateLost" value={formData.dateLost} onChange={handleChange} required className="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:border-blue-500 cursor-pointer" />
            </div>
          </div>
          ...
          <div className="flex justify-center gap-4 mt-8">
            ...
            <button type="submit" disabled={isSubmitting} className={`px-12 py-3 bg-[#7B1C1C] hover:bg-[#5a1414] text-white font-semibold rounded-lg transition ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}>
