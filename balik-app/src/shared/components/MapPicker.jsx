...
function LocationMarker({ position, setPosition, setAddress }) {
  ...
  // Removed the Marker component to remove the broken icon/text
  return null;
}
    ...
    <div className="relative h-full flex flex-col gap-4 p-6">
      ...
      <div className="relative z-[1000]">
        <div className="relative group">
          <button 
            type="button"
            onClick={() => !isSearching && performSearch(searchQuery)}
            className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-600 transition-colors hover:text-blue-600 z-10"
          >
          ...
          {searchQuery && (
            <button 
              ...
              className="cursor-pointer absolute right-4 top-1/2 -translate-y-1/2 p-1 hover:bg-slate-100 rounded-full text-slate-400 transition-all"
            >
              <X size={16} />
            </button>
          )}
        ...
        {showSuggestions && suggestions.length > 0 && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
            <div className="max-h-[300px] overflow-y-auto seamless-scrollbar">
      ...
      <div className="flex-1 rounded-2xl overflow-hidden border-2 border-slate-100 relative group">
        ...
        {/* Map Overlay (Crosshair center) - Always visible or when picking */}
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-[400]">
          <div className="w-12 h-12 flex items-center justify-center text-blue-600/40">
            <div className="w-1.5 h-1.5 bg-blue-600 rounded-full" />
            <div className="absolute w-8 h-[2px] bg-blue-600/30" />
            <div className="absolute h-8 w-[2px] bg-blue-600/30" />
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 p-4 bg-slate-50 rounded-2xl border border-slate-200">
        ...
        <div className="flex gap-3">
          <button 
            onClick={onClose}
            className="cursor-pointer flex-1 py-3 px-4 rounded-xl font-bold text-slate-600 hover:bg-slate-200 transition-colors text-sm"
          >
            Cancel
          </button>
          <button 
            ...
            className={`flex-1 py-3 px-4 rounded-xl font-bold text-white transition-all text-sm ${
              position 
                ? 'cursor-pointer bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-200 active:scale-95' 
